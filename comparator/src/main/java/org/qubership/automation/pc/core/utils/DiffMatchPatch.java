...
public class DiffMatchPatch {
        ...
        while (thisDiff != null) {
            switch (thisDiff.operation) {
                ...
                case EQUAL:
                    ...
                    break;
                default:
                    // No action needed for unknown operations
                    break;
            }
    ...
    protected LinkedList<Diff> diffBisect(String text1, String text2, long deadline) {
        ...
        int valueLength = 2 * maxD;
        int[] v1 = new int[valueLength];
        int[] v2 = new int[valueLength];
        ...
        for (int d = 0; d < maxD; d++) {
            ...
            for (int k1 = -d + k1start; k1 <= d - k1end; k1 += 2) {
                ...
                if (x1 > text1Length) {
                    ...
                    if (k2Offset >= 0 && k2Offset < valueLength && v2[k2Offset] != -1) {
            ...
            for (int k2 = -d + k2start; k2 <= d - k2end; k2 += 2) {
                ...
                if (x2 > text1Length) {
                    ...
                    if (k1Offset >= 0 && k1Offset < valueLength && v1[k1Offset] != -1) {
                        int x1 = v1[k1Offset];
    ...
    protected String[] diffHalfMatch(String text1, String text2) {
        ...
        String[] hm1 = diffGalfMatchI(longtext, shorttext,
                (longtext.length() + 3) / 4);
        // Check again based on the third quarter.
        String[] hm2 = diffGalfMatchI(longtext, shorttext,
                (longtext.length() + 1) / 2);
        String[] hm;
     ...
     */
    private String[] diffGalfMatchI(String longtext, String shorttext, int i) {
    ...
    public String diffToDelta(LinkedList<Diff> diffs) {
        ...
        for (Diff diff : diffs) {
            switch (diff.operation) {
                case INSERT:
                    try {
                        text.append("+").append(URLEncoder.encode(diff.text, "UTF-8")
                                .replace('+', ' ')).append("\t");
     ...
     * @param diffs Array of Diff objects for text1 to text2.
     * @return LinkedList of Patch objects.
     * @deprecated Prefer patchMake(String text1, {@code LinkedList<Diff>} diffs).
     */
    public LinkedList<Patch> patchMake(String text1, String text2,
                                       LinkedList<Diff> diffs) {
        return patchMake(text1, diffs);
            ...
            switch (diff.operation) {
                case INSERT:
                    ...
                    postpatchText = postpatchText.substring(0, charCount2)
                            + diff.text + postpatchText.substring(charCount2);
                    break;
                case DELETE:
                    patch.length1 += diff.text.length();
                    patch.diffs.add(diff);
                    postpatchText = postpatchText.substring(0, charCount2)
                            + postpatchText.substring(charCount2 + diff.text.length());
                    break;
                case EQUAL:
                    if (diff.text.length() <= 2 * patchMargin
                            && !patch.diffs.isEmpty() && diff != diffs.getLast()) {
    ...
    /**
     * Given an LinkedList of patches, return another LinkedList that is identical.
        ...
        for (Patch patch : patches) {
            ...
            if (startLoc == -1) {
                ...
                if (text1.equals(text2)) {
                    ...
                    text = text.substring(0, startLoc) + diffText2(patch.diffs)
    ...
    public static class Patch {
        ...
        public String toString() {
            ...
            for (Diff diff : this.diffs) {
                ...
                try {
                    text.append(URLEncoder.encode(diff.text, "UTF-8").replace('+', ' '))
                            .append("\n");
                } catch (UnsupportedEncodingException e) {
                    // Not likely on modern system.
                    throw new Error("This system does not support UTF-8.", e);
