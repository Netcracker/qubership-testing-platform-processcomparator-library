...
class CmdLineInfoPart implements ISVNInfo {
    ...
    /**
     * Get true when the info is versioned.
     *
    ...
    /**
     * Parses the output of the {@code svn info} command and splits it into distinct information blocks.
     * </p>
     * The output from {@code svn info} can contain multiple sections, each representing information
     * about a versioned or unversioned resource. This method identifies each section by detecting lines
     * that begin with {@code "Path:"} or end with {@code ":  (Not a versioned resource)"}.
     *
     * @param infoLines The full text output from the {@code svn info} command.
     * @return An array of strings, where each element represents an individual info block.
     */
    ...
    /**
     * Factory method to create a representation of unversioned resource information.
     * </p>
     * This method is typically used when {@code svn info} identifies a path that is not under version control.
     * It creates a {@link CmdLineInfoPartUnversioned} object to encapsulate this state.
     *
     * @param path The file system path that is not under version control.
     * @return A new instance of {@link CmdLineInfoPartUnversioned} for the given path.
     */
